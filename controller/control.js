...
const seeAll_like_dislike = async(req,res)=>{
  try {
    ...
    res.render('seelike-dislike',{posts:posts})
    console.log(posts);
  } catch (error) {
    console.log(error);
...
const my_call = async (req, res, next) => {
    try {
      ...
      if (rows.length > 0) {
        ...
        res.render('navbarPage', {data:rows ,postdata:data});
      }else {
...
const chatpage = (req,res)=>{
  res.render('ChatPage',{error:null})
}

module.exports={chatpage,seeAll_like_dislike,likeDislike,likePost,dislikePost,uploadImageProfile,createUserPost,createPost,homepage,deleteuserdata,profileImage,updatePage,loginPage,my_call,home,createUser,read_all_data,update,deletedata,delete_all_data,loginUser, Logout};
